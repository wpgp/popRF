...
#' @param covariates details of the covariates
...
merge_covariates <- function(input.countries,
  ...
  log_info("MSG",
           paste0("Start merging covariates"),
           verbose=verbose,
           log=log) 
  ...
  for ( i in 1:length(covariates.var.names) ) {  
    ...
    dstfile <- file.path(rfg.countries.merged,
                         paste0(rfg.countries.tag,
                                "_",
                                cname,
                                ".tif"))
    ...
    if (!file.exists(dstfile)){
      ...
      #          of = "GTiff",
      #          dstfile=dstfile,
      #          co=c("COMPRESS=LZW", "BLOCKXSIZE=512",
      #           "BLOCKYSIZE=512", "TILED=YES", "BIGTIFF=YES"),
  ...
  #log_info("MSG", paste0("Completed merging covariates"),
